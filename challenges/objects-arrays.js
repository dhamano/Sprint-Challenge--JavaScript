...
console.log('trex.weight:',trex.weight);

// What was the diet of a velociraptor?
console.log('veloc.diet:',veloc.diet);

// How long was a stegosaurus?
console.log('stego.length:',stego.length);

// What time period did tyrannosaurus live in?
console.log('trex.period:',trex.period);
...
/*
for(let i=0; i<graduates.length; i++) {
  if(universities[i].substring(0, 3) === "Uni") {
    uni.push(universities[i]);
  }
}
//*/
for(let i=0; i<graduates.length; i++) {
  let uniFound = universities[i].toLowerCase().search('uni');
  if(uniFound >= 0) {
		uni.push(universities[i]);
	}
}
...
*/
const animalNames = [];
zooAnimals.forEach(arrayItem => animalNames.push([`Name: ${arrayItem.animal_name}, Scientific: ${arrayItem.scientific_name}.`]));
...
const lowerCase = zooAnimals.map(arrayItem => arrayItem.animal_name.toLowerCase());
...
*/
const largerPopulation = zooAnimals.filter(arrayItem => arrayItem.population < 5);
console.log('largerPopulation',largerPopulation);
...
*/
const populationTotal = zooAnimals.reduce((populationSum, arrayItem) => { return populationSum += arrayItem.population; }, 0);
